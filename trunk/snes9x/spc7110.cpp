...
int decompack_size=0;	    // hack for PSP - Andon
...
#ifdef PSP
				decompack->binfiles [i] = (uint8 *)malloc (buf.st_size * sizeof (uint8));
#else
...
#ifdef PSP
					decompack->binfiles [i] = (uint8 *) malloc (sizeof (uint8) * buf.st_size);
#else
					decompack->binfiles[i]=new uint8[buf.st_size];
#endif
				}
