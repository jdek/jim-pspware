...
static uint8	malloc_tmp1[0x8000];
static uint8	malloc_tmp2[0x80000];
static uint8	malloc_tmp3[0x10000];

static uint8	_RAM[0x20000];
static uint8	_SRAM[0x20000];
static uint8	_VRAM[0x10000];
static uint8	_ROM[CMemory::MAX_ROM_SIZE + 0x200 + 0x8000];
static uint8	_BSRAM[0x80000];

static uint8	IPPU_TileCache_TILE_2BIT[MAX_2BIT_TILES * 128];
static uint8	IPPU_TileCache_TILE_4BIT[MAX_4BIT_TILES * 128];
static uint8	IPPU_TileCache_TILE_8BIT[MAX_8BIT_TILES * 128];
...
bool8 CMemory::Init ()
{
    ...
    IPPU.TileCached [TILE_2BIT] = (uint8 *) IPPU_TileCached_TILE_2BIT;
    IPPU.TileCached [TILE_4BIT] = (uint8 *) IPPU_TileCached_TILE_4BIT;
    IPPU.TileCached [TILE_8BIT] = (uint8 *) IPPU_TileCached_TILE_8BIT;
#else
...
/*		ROM_INFO stRomInfo;
		...
		// Unzip展開する
		nRet = Unzip_execExtract(filename, (unsigned long)&stRomInfo);
		if (nRet != UZEXR_OK) {
		...
		calc_size = (FileSize / 0x2000) * 0x2000;
			...
			{
				memmove (stRomInfo.p_rom_image, stRomInfo.p_rom_image + 512, calc_size);
				HeaderCount = 1;
				FileSize -= 512;
			}
