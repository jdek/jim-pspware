...
static int cmd_cm(void *internal, mpl__snd_mixer_t ** mixer, mpl__snd_mixer_opt_t opt)
{
...
static int cmd_dm(void *internal, mpl__snd_mixer_t * mixer)
{
    return 0;
}

static int cmd_goo(void *internal, mpl__snd_dev_output_t * opt)
{
    opt->channels = 2;
    opt->format = 2;
    opt->freq = 44100;
...
static void setparams(int ch, int freq, int bits)
{
    format.nChannels = ch;
