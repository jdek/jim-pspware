...
// PSP ALIGNED
#define READ_RGB16_Y(ROW, UVID, C1,C2,C3,C4)	\
	rgb = *(x_ptr + ((ROW)*x_stride) + 0);	\
	rgb |= (*(x_ptr + ((ROW)*x_stride) + 1)) << 8;	\
	b##UVID += b = C1##_B(rgb);				\
	g##UVID += g = C1##_G(rgb);				\
	r##UVID += r = C1##_R(rgb);				\
	y_ptr[(ROW)*y_stride+0] =				\
		(uint8_t) ((FIX_IN(Y_R_IN) * r + FIX_IN(Y_G_IN) * g +	\
					FIX_IN(Y_B_IN) * b) >> SCALEBITS_IN) + Y_ADD_IN;	\
	rgb = *(x_ptr + ((ROW)*x_stride) + 2);	\
	rgb |= (*(x_ptr + ((ROW)*x_stride) + 3) << 8);	\
	b##UVID += b = C1##_B(rgb);				\
	g##UVID += g = C1##_G(rgb);				\
	r##UVID += r = C1##_R(rgb);				\
	y_ptr[(ROW)*y_stride+1] =				\
...
#define WRITE_RGB16(ROW,UV_ROW,C1)	\
	...
	g[ROW] = (g[ROW] & 0x7) + ((rgb_y - g_uv##UV_ROW) >> SCALEBITS_OUT);	\
	r[ROW] = (r[ROW] & 0x7) + ((rgb_y + r_v##UV_ROW) >> SCALEBITS_OUT);		\
	*(x_ptr+((ROW)*x_stride)+0) = C1(r[ROW], g[ROW], b[ROW]);	\
	*(x_ptr+((ROW)*x_stride)+1) = C1(r[ROW], g[ROW], b[ROW]) >> 8;	\
	rgb_y = RGB_Y_tab[ y_ptr[y_stride*(ROW) + 1] ];				\
	b[ROW] = (b[ROW] & 0x7) + ((rgb_y + b_u##UV_ROW) >> SCALEBITS_OUT);		\
	g[ROW] = (g[ROW] & 0x7) + ((rgb_y - g_uv##UV_ROW) >> SCALEBITS_OUT);	\
	r[ROW] = (r[ROW] & 0x7) + ((rgb_y + r_v##UV_ROW) >> SCALEBITS_OUT);		\
	*(x_ptr+((ROW)*x_stride)+2) = C1(r[ROW], g[ROW], b[ROW]);	\
	*(x_ptr+((ROW)*x_stride)+3) = C1(r[ROW], g[ROW], b[ROW]) >> 8;
