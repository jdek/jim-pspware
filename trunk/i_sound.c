...
struct sound_channel sndchans[NUM_CHANNELS];
  ...
  int		seperation )
{
	...
	sndchans[slot].is_playing = 1;
	sndchans[slot].ptr = channels[slot];
	sndchans[slot].len = lengths[sfxid];
...
// This function currently supports only 16bit.
//
void I_UpdateSound(void* buf, unsigned int length)
{
  ...
  int				chan;
    ...
    leftout = (signed short *)buf;
    rightout = ((signed short *)buf)+1;
    step = 2;
...
void sound_callback(void *buf, unsigned int _reqn)
{
	int j,k;
	int l,i;
	char *p;
