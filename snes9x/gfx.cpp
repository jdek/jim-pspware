...
#define FIX_INTERLACE(SCREEN, DO_DEPTH, DEPTH) \
    if (IPPU.DoubleHeightPixels && ((PPU.BGMode != 5 && PPU.BGMode != 6) || !IPPU.Interlace)) \
        ...
        { \
        	memmove (SCREEN + ((y << 1) + 1) * GFX.Pitch2, \
                     SCREEN + (y << 1) * GFX.Pitch2, \
                     GFX.Pitch2); \
            if(DO_DEPTH){ \
            	memmove (DEPTH + ((y << 1) + 1) * (GFX.PPLx2>>1), \
                         DEPTH + y * GFX.PPL, \
                         GFX.PPLx2>>1); \
            } \
        }
...
bool8 S9xGraphicsInit ()
{
    ...
    {
#ifdef PSP
    memset(GFX_X2, 0, sizeof(GFX_X2));
	GFX.X2 = (uint16 *) GFX_X2;
    memset(GFX_ZERO_OR_X2, 0, sizeof(GFX_ZERO_OR_X2));
	GFX.ZERO_OR_X2 = (uint16 *) GFX_ZERO_OR_X2;
    memset(GFX_ZERO, 0, sizeof(GFX_ZERO));
	GFX.ZERO = (uint16 *) GFX_ZERO;
#else
...
#ifdef MK_DEBUG_RTO
	if(Settings.BGLayering) fprintf(stderr, "Priority rotation=%d, OAMAddr=%d -> ", PPU.OAMPriorityRotation, (PPU.OAMAddr << 1) | (PPU.OAMFlip&1));
#endif
...
if(Settings.BGLayering) {
			...
			int TileLine = ((GFX.OBJLines[Y].OBJ[I].Line&7) << 3);
			int TileInc = 1;
...
void S9xUpdateScreen ()
{
		...
		IPPU.Interlace || IPPU.DoubleHeightPixels))
    {
		...
		if (IPPU.Interlace && !IPPU.DoubleHeightPixels)
		{
			...
			{
				memmove (GFX.Screen + (y << 1) * GFX.Pitch2,
					GFX.Screen + y * GFX.Pitch2,
					GFX.Pitch2);
				memmove (GFX.Screen + ((y << 1) + 1) * GFX.Pitch2,
					GFX.Screen + y * GFX.Pitch2,
					GFX.Pitch2);
			}
		}
