...
#define HINT_KEYSPEED "Adjust the paddle speed for movement by keys. (pixels per second)"
...
#define HINT_DIFF "KIDS:#Lives: 9 (max. 12)#Paddle Size: 252 (max. 396)#Ball Speed: 100-200#Bonus: -50% (no maluses, no bad speech)##"\
...
#define HINT_START_LBR "Play the original LBreakout2 levelset."
#define HINT_START_CUSTOM "Play the additional levelset selected below."
...
#define HINT_EDITOR_EDIT "Click here to edit the levelset chosen above.##NOTE: To switch to the bonus set you'll have to right-click on the brick set and vice versa!"
#define HINT_EDITOR_DELETE "Click here to delete the chosen levelset (after confirmation)."
#define HINT_KEYS "If you use the keyboard for input these are the keys for moving the paddle ('Paddle Turbo' will double the speed while pressed.), firing balls/shots and returning the ball to paddle after a timeout of ten seconds.##Note: If balls are fired at a random angle (which has become the default) "\
...
#define HINT_MOD "This modifies the relative motion of the mouse. (percent) "\
		 "The higher this value is the faster the mouse moves."
#define HINT_INVERT "If for some reason your mouse movement is inverted you can fix it with this option."
...
#define HINT_APPLY "Click here to apply the theme you selected above. Please check the manual at lgames.sf.net for information on how to " \
                   "create a theme."
...
#define HINT_LOCAL_PORT "Network communication is handled by a single UDP socket which will be "\
	"bound to this port number."
#define HINT_RANDOM_ANGLE "If '50' the ball is shot at an angle of 50 degrees to the left or "\
...
#define HINT_MAXBALLSPEED "While you keep the middle mouse button pressed (or the acceleration key) "\
        "all balls will go this velocity. (in pixels per second)"
#define HINT_BONUS_LEVELS "If on, normal levelsets will have a random bonus level every four normal levels.##"\
...
#define HINT_FREAKOUT "Play *ALL* available levels in a random order."
#define HINT_LBREAKOUT1 "The complete levelset of LBreakout1 with just minor modifications."
#define HINT_JUMPING_JACK "There is only one brick with a limited lifetime. Your task is to hit "\
...
#define HINT_OUTBREAK "Your task is to prevent an outbreak by clearing all bricks. Every few seconds "\
                      ...
                      "MAKE EXCESSIVE USE OF YOUR MIDDLE MOUSE BUTTON! Otherwise you won't "\
                      "stand a chance as these levels require a lot of skill!"
#define HINT_BARRIER  "This time you have to crush through a wall and hit some bricks behind it. "\
                      ...
                      "MAKE EXCESSIVE USE OF YOUR MIDDLE MOUSE BUTTON! Otherwise you won't "\
                      "stand a chance as these levels require a lot of skill!"
#define HINT_SITTING_DUCKS "There are eight bricks and the ball is attached in the middle at the "\
                           ...
                           "side of the screen.##"\
                           "Loosing your ball means going to the next level."
#define HINT_HUNTER "Your task is to hunt down bricks by moving a chaos brick indirectly. Therefore "\
                    ...
                    "MAKE EXCESSIVE USE OF YOUR MIDDLE MOUSE BUTTON! Otherwise you won't "\
                    "stand a chance as these levels require a lot of skill!"
#define HINT_DEFENDER "Stop the invading brick waves! Each waves consists of 50 bricks which will get "\
...
void cb_update_levelset_hint()
{
    ...
    if ( STRCMP( levelset_names_local[config.levelset_id_local], TOURNAMENT ) ) {
    ...
    else if ( STRCMP( levelset_names_local[config.levelset_id_local], "!JUMPING_JACK!" ) ) {
        sprintf( hint_levelset, HINT_JUMPING_JACK );
    }
    else if ( STRCMP( levelset_names_local[config.levelset_id_local], "!OUTBREAK!" ) ) {
        sprintf( hint_levelset, HINT_OUTBREAK );
    }
    else if ( STRCMP( levelset_names_local[config.levelset_id_local], "!BARRIER!" ) ) {
        sprintf( hint_levelset, HINT_BARRIER );
    }
    else if ( STRCMP( levelset_names_local[config.levelset_id_local], "!SITTING_DUCKS!" ) ) {
        sprintf( hint_levelset, HINT_SITTING_DUCKS );
    }
    else if ( STRCMP( levelset_names_local[config.levelset_id_local], "!HUNTER!" ) ) {
        sprintf( hint_levelset, HINT_HUNTER );
    }
    else if ( STRCMP( levelset_names_local[config.levelset_id_local], "!INVADERS!" ) ) {
        ...
        {
            sprintf( hint_levelset, "No info available." );
        }
...
void manager_create()
{
    ...
    char *str_diff[] = { "Kids", "Easy", "Medium", "Hard" };
    /*
    main:
        new game:
            start game
...
//    menu_add( _main, item_create_link    ( "Select Theme", HINT_THEME, theme ) );
    ...
    /* game */
    item_resume_0 = item_create_action( "Resume Last Game", "???", ACTION_RESUME_0 );
    menu_add( game, item_resume_0 );
    slot_update_hint( 0, item_resume_0->hint );
    menu_add( game, item_create_action( "Start Original Set", HINT_START_LBR, ACTION_PLAY_LBR ) );
    menu_add( game, item_create_action( "Start AddOn", HINT_START_CUSTOM, ACTION_PLAY_CUSTOM ) );
    //menu_add( game, item_create_separator( "" ) );
    item_levelset = item_create_switch_x( "AddOn:", "", &config.levelset_id_local, levelset_names_local, levelset_count_local );
    item_levelset->callback = cb_update_levelset_hint;
    cb_update_levelset_hint(); /* initiate first hint */
    menu_add( game, item_levelset );
    menu_add( game, item_create_switch_x( "Difficulty:", HINT_DIFF, &config.diff, str_diff, DIFF_COUNT ) );
    menu_add( game, item_create_separator( "" ) );
    ...
    keys[4] = item_create_key( "Paddle Turbo:", HINT_KEYS, &config.k_turbo, filter );
    keys[5] = item_create_key( "Ball Turbo:", HINT_KEYS, &config.k_maxballspeed, filter );
    keys[6] = item_create_key( "Return:", HINT_KEYS, &config.k_return, filter );
//    keys[6] = item_create_key( "Warp:", HINT_KEYS, &config.k_warp, filter );
    ...
    item = item_create_range( "Key Speed:", HINT_KEYSPEED, &config.i_key_speed, 100, 1000, 50 );
    item->callback = cb_set_keyspeed;
    menu_add( cont, item );
    item = item_create_range( "Motion Modifier:", HINT_MOD, &config.i_motion_mod, 40, 160, 5 );
    ...
    menu_add( adv,
	item_create_switch( "Ball Fire Angle:", 
    ...
    menu_add( adv, item_create_edit( "Local UDP Port:", HINT_LOCAL_PORT, config.local_port, 6 ) );
    item = item_create_range( "Acc. Ball Speed:", HINT_MAXBALLSPEED, &config.maxballspeed_int1000, 600, 900, 20 );
    item->callback = cb_maxballspeed;
    menu_add( adv, item );
    menu_add( adv, item_create_switch( "Add Bonus Levels:", HINT_BONUS_LEVELS, &config.addBonusLevels, "Off", "On" ) );
