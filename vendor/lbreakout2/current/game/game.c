...
static void game_setup_title_and_author( Game *game, Level *level )
{
  switch (level->type)
    ...
    case LT_JUMPING_JACK:
      snprintf( game->title, 32, "Jumping Jack" );
      snprintf( game->author, 32, "Bonus Level" );
      break;
    case LT_OUTBREAK:
        snprintf( game->title, 32, "Outbreak" );
        snprintf( game->author, 32, "Bonus Level" );
        break;
    case LT_BARRIER:
        snprintf( game->title, 32, "Barrier" );
        snprintf( game->author, 32, "Bonus Level" );
        break;
    case LT_SITTING_DUCKS:
        snprintf( game->title, 32, "Sitting Ducks" );
        snprintf( game->author, 32, "Bonus Level" );
        break;
    case LT_HUNTER:
        snprintf( game->title, 32, "Hunter" );
        snprintf( game->author, 32, "Bonus Level" );
        break;
    case LT_DEFENDER:
        snprintf( game->title, 32, "Defender" );
        snprintf( game->author, 32, "Bonus Level" );
        break;
    default:
      snprintf( game->title, 32, "Unknown Level Type" );
