...
void client_connect( GuiWidget *widget, GuiEvent *event )
{
	...
	msg_write_string( "unused" ); /* passwd */
	
	while ( attempt < 3 ) {
		client_printf_chatter( 1, "%s: %s...", 
			config.server, 
			attempt==0?"connecting":"retry" );
		...
		while ( net_recv_packet() ) {
			...
			switch ( type ) {
				case MSG_LOGIN_OKAY:
					...
					client_printf_chatter( 1, "%s: connected!", config.server );
					client_is_connected = 1;
					return;
				case MSG_ERROR:
					client_printf_chatter( 1, "ERROR: connection refused: %s",
						msg_read_string() );
					return;
			}
		}
	...
	client_add_chatter( "ERROR: server does not respond", 1 );
#endif
}
    ...
    GuiWidget *widget, GuiEvent *event )
{
	...
	if ( client_user ) {
		...
		client_add_chatter( "You must select a user to whisper!", 1 );
#endif
}
...
void client_challenge( GuiWidget *widget, GuiEvent *event )
{
	...
	/* everything valid? */
	if ( client_user == 0 ) {
		client_popup_info( "You must select a user for a challenge." );
		return;
	}
	if ( client_levelset == 0 ) {
		client_popup_info( "You must select a levelset for a challenge." );
		return;
	}
	if ( client_user->id == client_id ) {
		client_popup_info( "You can't challenge yourself." );
		return;
	}
	...
	client_popup_info( "You have challenged %s. Let's see what (s)he says...", mp_peer_name );
