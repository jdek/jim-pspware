...
\
				...
				R = (p1*(c1 & Rmask) + p2*(c2 & Rmask) + p3*(c3 & Rmask) + p4*(c4 & Rmask))>>13;\
				G = (p1*(c1 & Gmask) + p2*(c2 & Gmask) + p3*(c3 & Gmask) + p4*(c4 & Gmask))>>13;\
				B = (p1*(c1 & Bmask) + p2*(c2 & Bmask) + p3*(c3 & Bmask) + p4*(c4 & Bmask))>>13;\
				if(Amask)\
					A = (p1*(c1 & Amask) + p2*(c2 & Amask) + p3*(c3 & Amask) + p4*(c4 & Amask))>>13;\
				\
				*(dst_row + x) = (R & Rmask) | (G & Gmask) | (B & Bmask) | (A & Amask);\
			} \
			sx += ctx;  /* Incremental transformations */ \
			sy -= sty; \
		} \
...
SDL_Rect sge_transformNorm(SDL_Surface *src, SDL_Surface *dst, float angle, float xscale, float yscale ,Uint16 px, Uint16 py, Uint16 qx, Uint16 qy, Uint8 flags)
{
	...
	float theta = angle*PI/180.0;  /* Convert to radians.  */
