...
codecStubs *decoder;
static unsigned char banner[] = "PSP Media Center v0.7 by John_K & adresd\0";
    ...
    while ((retval > 0) && (count < 99)) {
      ...
      if (retval > 0) {
        if (dirent.name[0] != 0) {
          ...
          if ((temp[0] >= 'A' ) && (temp[0] <= 'Z')) temp[0] += 'a'-'A';
          if ((temp[1] >= 'A' ) && (temp[1] <= 'Z')) temp[1] += 'a'-'A';
          if ((temp[2] >= 'A' ) && (temp[2] <= 'Z')) temp[2] += 'a'-'A';
