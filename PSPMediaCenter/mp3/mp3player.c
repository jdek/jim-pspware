...
unsigned char		InputBuffer[INPUT_BUFFER_SIZE+MAD_BUFFER_GUARD],
					...
					*GuardPtr=NULL;
const unsigned char	*OutputBufferEnd=OutputBuffer+OUTPUT_BUFFER_SIZE*2;
int			Status=0, i;
...
static void MP3PlayCallback(short *_buf, unsigned long numSamples)
{
	...
	if (m_bPlaying == TRUE) { //  Playing , so mix up a buffer
		...
		while (samplesOut < numSamples) {
			...
			*/
			...
			/*if(ReadSize<=0)
			{
				//printf("%s: read error on bit-stream (%s - %d)\n", ProgName, error_to_string(errno), errno);
				//	Status=1;
...
void MP3Play_Init(int channel)
{
  ...
  m_bPlaying = FALSE;
  AudioSetChannelCallback(modplayint_channel,MP3PlayCallback);
