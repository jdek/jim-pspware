...
#-----------------------------------------------------------------------
# Copyright 2001-2005, pspdev - http://www.pspdev.org
...
{
  stubs->init = AHXPlayer_Init;
  stubs->load = AHXPlayer_Load;
  stubs->play = AHXPlayer_Play;
  stubs->pause = AHXPlayer_Pause;
...
int AHXPlayer_Load(char *filename)
{
  ...
  // Load into AHX
  AHXPlayer_LoadSongBuffer(data, (int) size);
...
void AHXVoice_CalcADSR(struct AHXVoice *pVoice)
{
  ...
  pVoice->ADSR.dVolume =
    (pVoice->Instrument->Envelope.dVolume - pVoice->Instrument->Envelope.aVolume) * 256 / pVoice->ADSR.dFrames;
  pVoice->ADSR.sFrames = pVoice->Instrument->Envelope.sFrames;
  pVoice->ADSR.rFrames = pVoice->Instrument->Envelope.rFrames;
  pVoice->ADSR.rVolume =
    (pVoice->Instrument->Envelope.rVolume - pVoice->Instrument->Envelope.dVolume) * 256 / pVoice->ADSR.rFrames;
}
...
void GenerateFilterWaveforms(char *Buffer, char *Low, char *High)
{
  ...
  for (temp = 0, freq = 8; temp < 31; temp++, freq += 3) {
    ...
    for (waves = 0; waves < 6 + 6 + 0x20 + 1; waves++) {	// 0 - 44
      char low_, high_;
      //mid = 0.f;
      //low = 0.f;
      midint = 0;
      ...
      for (i = 0; i <= LengthTable[waves]; i++) {
        ...
        highint = (((int) (a0[i])) << FIXEDPOINT) - midint - lowint;
        clipint(&highint);
        midint += (highint * (freint >> (FIXEDPOINT / 2)) >> (FIXEDPOINT / 2));
        clipint(&midint);
        lowint += (midint * (freint >> (FIXEDPOINT / 2)) >> (FIXEDPOINT / 2));
        clipint(&lowint);
      }
      for (i = 0; i <= LengthTable[waves]; i++) {
        //high = a0[i] - mid - low; clip(&high);
...
int AHXPlayer_LoadSongBuffer(unsigned char *Buffer, int Len)
{
  ...
  for (i = 0; i < Song.SubsongNr; i++) {
    if (SBPtr - SongBuffer > SongLength)
      return 0;
  ...
  for (i = 1; i < Song.InstrumentNr + 1; i++) {
    ...
    strcpy(Instruments[i].Name, NamePtr);
    NamePtr += strlen(NamePtr) + 1;
    if (SBPtr - SongBuffer > SongLength)
      return 0;
    ...
    SBPtr += 22;
    Instruments[i].PList.Entries =
      (struct AHXPListEntry *) malloc(sizeof(struct AHXPListEntry) * Instruments[i].PList.Length);
...
void AHXPlayer_ProcessFrame(int v)
{
  ...
  if (Voices[v].NoteCutOn) {
    if (Voices[v].NoteCutWait <= 0) {
      ...
      if (Voices[v].HardCutRelease) {
        Voices[v].ADSR.rVolume =
          -(Voices[v].ADSRVolume - (Voices[v].Instrument->Envelope.rVolume << 8)) / Voices[v].HardCutReleaseF;
  ...
  if (Voices[v].Instrument && Voices[v].PerfCurrent < Voices[v].Instrument->PList.Length) {
    if (--Voices[v].PerfWait <= 0) {
      ...
      for (i = 0; i < 2; i++)
        AHXPlayer_PListCommandParse(v, Voices[v].PerfList->Entries[Cur].FX[i],
        Voices[v].PerfList->Entries[Cur].FXParam[i]);
      //GetNote
  ...
  if (Voices[v].PeriodPerfSlideOn) {
    ...
    if (Voices[v].PeriodPerfSlidePeriod)
      Voices[v].PlantPeriod = 1;
  }
  ...
  //AudioInitVolume
  Voices[v].AudioVolume =
    ((((((((Voices[v].ADSRVolume >> 8) * Voices[v].NoteMaxVolume) >> 6) * Voices[v].PerfSubVolume) >> 6) *
    Voices[v].TrackMasterVolume) >> 6) * MainVolume) >> 6;
}
  ...
  if (Voices[v].NewWaveform) {
    if (Voices[v].Waveform == 4 - 1) {
      ...
      for (i = 0; i < WaveLoops; i++)
        memcpy(&Voices[v].VoiceBuffer[i * 4 * (1 << Voices[v].WaveLength)], Voices[v].AudioSource,
        4 * (1 << Voices[v].WaveLength));
    }
...
void AHXPlayer_PlayIRQ()
{
  ...
  if (StepWaitFrames <= 0) {
    ...
    for (i = 0; i < 4; i++)
      AHXPlayer_ProcessStep(i);
    StepWaitFrames = Tempo;
  }
  //DoFrameStuff
  for (i = 0; i < 4; i++)
    AHXPlayer_ProcessFrame(i);
  PlayingTime++;
    ...
    while (samples_to_mix) {
      ...
      if (Oversampling) {
        for (i = 0; i < thiscount; i++) {
          offset = pos[v] >> 16;
          sample1 = (int) VolTab[(int) Voices[v].VoiceBuffer[offset]];
          sample2 = (int) VolTab[(int) Voices[v].VoiceBuffer[offset + 1]];
          frac1 = pos[v] & ((1 << 16) - 1);
          frac2 = (1 << 16) - frac1;
        ...
        for (i = 0; i < thiscount; i++) {
          (*mb)[mixpos++] += (int) VolTab[(int) Voices[v].VoiceBuffer[pos[v] >> 16]];
          pos[v] += delta;
        }
      }
    }			// while
...
static void AHXPlayCallback(short *_buf, unsigned long length)
{
  if (m_bPlaying == 1) {
    while (tempmixleft < length) {	//  Not enough in buffer, so we must mix more
      ...
      tempmixleft += BlockLen2;
    }

    if (tempmixleft >= length) {	//  Buffer has enough, so copy across
      int count;
      short *_buf2;
      for (count = 0; count < length; count++) {
        _buf2 = _buf + count + count;
        // Double up for stereo
        *(_buf2) = tempmixbuf[count];
        *(_buf2 + 1) = tempmixbuf[count];
      }
      //  Move the pointers
      tempmixleft -= length;
      //  Now shuffle the buffer along
      for (count = 0; count < tempmixleft; count++)
        tempmixbuf[count] = tempmixbuf[length + count];
    }
