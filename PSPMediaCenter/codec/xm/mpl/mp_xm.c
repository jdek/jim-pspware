...
static int linear_note_to_period(int note, int fine_tune)
{
    return (int) (7680.0f - note * 64.0f - fine_tune * 0.5f);
}

static float32 linear_period_to_freq(int per)
{
    return (float32) (8363.0 * pow(2.0, (4608.0 - per) * 0.00130208333333));
}

static int note_to_period(int note, int fine_tune)
{
    float32 period, diff;
...
static float32 period_to_freq(int per)
{
    return 14317056.0f / per;
}

static mpl__xm_cell_t *get_row(mpl__mp_xm_t * pl)
{
...
static void update_ctrl(mpl__mp_xm_t * pl)
{
	...
	if (p_ch->ctrl & MPL__XM_CTRL_START) {
	    ...
	    mpl__snd_mixer_play_channel(pl->mixer, cur, pl->smp_handle[p_ch->p_smp->index], freq, vol, pan, 0.5f,
					p_ch->smp_offset, MPL__SND_DEV_DIR_FORWARDS);
...
static void do_vibrato(mpl__mp_xm_chan_t * p_ch, int exp, int update_pos)
{
    int delta, temp;
...
static void do_inst_vibrato(mpl__mp_xm_chan_t * p_ch)
{
    int delta;
...
static void do_tremolo(mpl__mp_xm_chan_t * p_ch)
{
    int delta;
	...
	break;
    case 1:
	delta = p_ch->tremolo_pos < 32 ? p_ch->tremolo_pos << 3 : 255 - (p_ch->tremolo_pos << 3);
	break;
    case 2:
	delta = 255;
	break;
...
static void do_porta(mpl__mp_xm_chan_t * p_ch)
{
...
static void do_tremor(mpl__mp_xm_chan_t * p_ch)
{
...
static void do_env_vol(mpl__mp_xm_chan_t * p_ch)
{
	...
	if ((p_ch->p_inst->vol_type & MPL__XM_ENV_LOOP) && pos == p_ch->p_inst->vol_loop_end) {
...
static void do_env_pan(mpl__mp_xm_chan_t * p_ch)
{
    mpl__xm_point_t *cur, *next;
    int pos, tick_inc = 1, divide;
	...
	if ((p_ch->p_inst->pan_type & MPL__XM_ENV_LOOP) && pos == p_ch->p_inst->pan_loop_end) {
...
static void do_vol_slide(mpl__mp_xm_chan_t * p_ch, int slide_value)
{
    int param_x, param_y;
...
static void do_vol(mpl__mp_xm_t * pl, mpl__mp_xm_chan_t * p_ch, int vol)
{
    if (vol < 0x10) {
	return;
    }
...
static void new_smp(mpl__mp_xm_chan_t * p_ch)
{
...
static void retrig(mpl__mp_xm_chan_t * p_ch)
{
    if (!p_ch->p_smp || p_ch->key == MPL__XM_NO_NOTE) {
	return;
    }
...
static void update_row(mpl__mp_xm_t * pl)
{
    int ch;
	...
	porta = cell->effect == MPL__XM_EFF_PORTA ||
	    cell->effect == MPL__XM_EFF_PORTA_VOL_SLIDE || (cell->volume & 0xf0) == MPL__XM_VOL_EFF_PORTA;
...
void update_effects(mpl__mp_xm_t * pl)
{
    int ch;
    mpl__xm_cell_t *row_cur, *cell;
    mpl__xm_t *xm;
	...
	case MPL__XM_EFF_ARPEGGIO:
		...
		if (p_ch->p_smp) {
			...
			p_ch->per_delta =
			    note_to_period(p_ch->real_key + param_x,
					   pl->smp_fine_tune[p_ch->p_smp->index]) - note_to_period(p_ch->real_key,
												   pl->
												   smp_fine_tune[p_ch->
														 p_smp->
														 index]);
		...
		if (p_ch->p_smp) {
			...
			p_ch->per_delta =
			    note_to_period(p_ch->real_key + param_y,
					   pl->smp_fine_tune[p_ch->p_smp->index]) - note_to_period(p_ch->real_key,
												   pl->
												   smp_fine_tune[p_ch->
														 p_smp->
...
/*				if (pl->tick_cur == param_y)
	...
	case MPL__XM_EFF_RETRIG_VOL_SLIDE:
		...
		break;
	    case 0x60:
		p_ch->vol = (p_ch->vol << 1) / 3;
		break;
	    case 0xE0:
		p_ch->vol = (p_ch->vol * 3) >> 1;
		break;
	    case 0x70:
		p_ch->vol >>= 1;
		break;
...
static void run_tick(mpl__mp_xm_t * pl, int param)
{
    if (!pl->xm->length) {
	return;
    }
...
static int mp_cmd_set_dev(void *internal_data, mpl__snd_dev_t * dev)
{
    mpl__mp_xm_t *pl = (mpl__mp_xm_t *) internal_data;
...
static int mp_cmd_play(void *internal_data)
{
    mpl__mp_xm_t *pl = (mpl__mp_xm_t *) internal_data;
...
static int mp_cmd_stop(void *internal_data)
{
    mpl__mp_xm_t *pl = (mpl__mp_xm_t *) internal_data;
...
static int mp_cmd_set_loop(void *internal_data, int loop)
{
    mpl__mp_xm_t *pl = (mpl__mp_xm_t *) internal_data;
...
static int mp_cmd_set_pos(void *internal_data, int pos)
{
    mpl__mp_xm_t *pl = (mpl__mp_xm_t *) internal_data;
...
static int mp_cmd_get_pos(void *internal_data)
{
    mpl__mp_xm_t *pl = (mpl__mp_xm_t *) internal_data;
...
static int mp_cmd_set_vol(void *internal_data, float32 vol)
{
    mpl__mp_xm_t *pl = (mpl__mp_xm_t *) internal_data;
...
static int mp_cmd_get_vol(void *internal_data, float32 * vol)
{
    mpl__mp_xm_t *pl = (mpl__mp_xm_t *) internal_data;
...
static int mp_cmd_set_option(void *internal_data, char *option, char *value)
{
    mpl__mp_xm_t *pl = (mpl__mp_xm_t *) internal_data;
...
static int mp_cmd_get_option(void *internal_data, char *option, char **value)
{
    mpl__mp_xm_t *pl = (mpl__mp_xm_t *) internal_data;
...
static int mp_cmd_get_proc(void *internal_data, char *name, void_func_t * proc)
{
    mpl__mp_xm_t *pl = (mpl__mp_xm_t *) internal_data;
